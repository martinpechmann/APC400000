# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\midi-remote-scripts\Push\DeviceNavigationComponent.py
...
class DeviceNavigationComponent(CompoundComponent):
    ...
    def set_exit_button(self, exit_button):
        if exit_button and not exit_button.is_momentary(): raise AssertionError
        self._on_exit_value.subject = exit_button
        self._update_exit_button()

    def set_enter_button(self, enter_button):
        if enter_button and enter_button.is_momentary(): raise AssertionError
