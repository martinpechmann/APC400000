# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\midi-remote-scripts\Push\PadSensitivity.py
...
class PadUpdateComponent(ControlSurfaceComponent):
    ...
    def __init__(self, all_pads = tuple(), parameter_sender = nop, default_profile = PadParameters(), update_delay = 0, *a, **k):
        if not find_if(lambda pad: pad < 0 or pad > 63, all_pads or []) == None: raise AssertionError
    ...
    def set_pad(self, pad, new_profile):
        ...
        if not new_profile in self._profile_count: raise AssertionError
