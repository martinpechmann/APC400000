# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\midi-remote-scripts\Push\NavigationNode.py
import Live.DrumPad
import Live.Song
import Live.Track
...
def make_navigation_node(model_object, is_entering = True, device_bank_registry = None):
    ...
    if model_object == None:
        ...
        if model_object.can_have_chains:
            ...
            if not device_bank_registry: raise AssertionError, 'Navigating a device needs a bank registry'
...
class SimpleDeviceNode(ModelNode):
    ...
    def __init__(self, device_bank_registry = None, *a, **k):
        ...
        if not device_bank_registry: raise AssertionError, 'Need a device bank registry.'
