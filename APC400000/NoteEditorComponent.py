# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\midi-remote-scripts\Push\NoteEditorComponent.py
...
class NoteEditorComponent(CompoundComponent, Subject):
    ...
    def set_selected_page_point(self, point):
        if not self.can_change_page: raise AssertionError
    ...
    def _get_step_start_time(self, x, y):
        ...
        if not in_range(x, 0, self._width): raise AssertionError
        if not in_range(y, 0, self._height): raise AssertionError
