# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\midi-remote-scripts\Push\NoteSettingsComponent.py
import math
...
class NoteSettingsComponent(ControlSurfaceComponent):
    ...
    def _add_setting(self, setting):
        if not len(self._settings) < 8: raise AssertionError, 'Cannot show more than 8 settings'
...
class NoteEditorSettingsComponent(ModesComponent):
    ...
    def __init__(self, grid_resolution = None, initial_encoder_layer = None, encoder_layer = None, settings_layer = None, *a, **k):
        ...
        if not encoder_layer: raise AssertionError
        self._bottom_data_sources = [ DisplayDataSource() for _ in xrange(8) ]
    ...
    def add_editor(self, editor):
        if editor == None: raise AssertionError
