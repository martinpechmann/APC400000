# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\midi-remote-scripts\Push\HandshakeComponent.py
"""
...
class HardwareIdentity(NamedTuple):
    ...
    def major_version(self):
        if not len(self.firmware) == 4: raise AssertionError
...
class MinimumFirmwareVersionElement(ToggleElement):
    ...
    def __init__(self, major_version = 0, minor_version = 0, wrapped_element = None, handshake_component = None, *a, **k):
        if wrapped_element is None: raise AssertionError
        if handshake_component is None: raise AssertionError
