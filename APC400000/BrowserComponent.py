# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\midi-remote-scripts\Push\BrowserComponent.py
...
class FullBrowserModel(BrowserModel):
    ...
    def _push_content_list(self):
        if self._num_contents < len(self._contents):
            ...
            if not self._num_contents == len(self._contents): raise AssertionError
    ...
    def _pop_content_list(self):
        if not self._num_contents > 1: raise AssertionError
        self._num_contents -= 1
    ...
    def _fit_content_lists(self, requested_lists):
        ...
        """
        if not requested_lists > 0: raise AssertionError
...
class PathBrowserQuery(BrowserQuery):
    ...
    def __init__(self, path = tuple(), *a, **k):
        ...
        if not path: raise AssertionError
        self.path = path
