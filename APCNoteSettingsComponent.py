...
class NoteEditorSettingsComponent(ModesComponent, APCMessenger):

    def __init__(self, grid_resolution = None, initial_encoder_layer = None, encoder_layer = None, automation_encoder_layer = None, settings_layer = None, *a, **k):
        ...
        self._mode_selector = self.register_component(ModesComponent())
        self._mode_selector.add_mode('note_settings', [partial(self._set_envelope_view_visible, False)], toggle_value=False)
        self._mode_selector.add_mode('automation', [partial(self._set_envelope_view_visible, True)], toggle_value=True)
        self._mode_selector.set_enabled(False)
        self._on_selected_mode.subject = self._mode_selector
        self._update_available_modes()
        self._mode_selector.selected_mode = 'note_settings'
        ...
        self._settings_modes.add_mode('automation', [self.settings, self._automation, AddLayerMode(self, automation_encoder_layer), self._show_clip_view])
        self._settings_modes.add_mode('note_settings', [self.settings, self._update_note_infos, self._show_clip_view])
        ...
        self._notes_encoders = None
        self._initial_notes_encoders = None
        self._parameter_encoders = None
        self._initial_parameter_encoders = None
    ...
    def set_initial_notes_encoders(self, encoders):
        self._initial_notes_encoders = encoders
        self._on_init_notes_encoder_touch.replace_subjects(encoders or [])
        self._on_init_notes_encoder_value.replace_subjects(encoders or [])
        self._try_immediate_show_settings()

    def set_notes_encoders(self, encoders):
        self._notes_encoders = encoders
        self._on_notes_encoder_touch.replace_subjects(encoders or [])
        self._on_notes_encoder_value.replace_subjects(encoders or [])
        self.settings.set_encoder_controls(encoders)

    def set_initial_parameter_encoders(self, encoders):
        self._initial_parameter_encoders = encoders
        self._on_init_parameter_encoder_touch.replace_subjects(encoders or [])
        self._on_init_parameter_encoder_value.replace_subjects(encoders or [])
        self._try_immediate_show_settings()

    def set_parameter_encoders(self, encoders):
        self._parameter_encoders = encoders
        self._on_parameter_encoder_touch.replace_subjects(encoders or [])
        self._on_parameter_encoder_value.replace_subjects(encoders or [])
    ...
    def _update_available_modes(self):
        if not self._automation.can_automate_parameters:
            self._mode_selector.pop_mode('automation')
    ...
    def _try_immediate_show_settings(self):
        if self.selected_mode == 'about_to_show' and any(imap(lambda e: e and e.is_pressed(), self._initial_notes_encoders or self._initial_parameter_encoders or [])):
            self._show_settings()
    ...
    def _on_selected_mode(self, mode):
        self._update_selected_setting(mode)

    @subject_slot_group('touch_value')
    def _on_init_notes_encoder_touch(self, value, encoder):
        self._show_settings()

    @subject_slot_group('value')
    def _on_init_notes_encoder_value(self, value, encoder):
        self._show_settings()

    @subject_slot_group('touch_value')
    def _on_notes_encoder_touch(self, value, encoder):
        if not value:
            self._try_hide_settings()

    @subject_slot_group('value')
    def _on_notes_encoder_value(self, value, encoder):
        self._notify_modification()

    @subject_slot_group('touch_value')
    def _on_init_parameter_encoder_touch(self, value, encoder):
        self._show_settings()

    @subject_slot_group('value')
    def _on_init_parameter_encoder_value(self, value, encoder):
        self._show_settings()

    @subject_slot_group('touch_value')
    def _on_parameter_encoder_touch(self, value, encoder):
        if not value:
            self._try_hide_settings()

    @subject_slot_group('normalized_value') # Adding a normalized_value_listener tells the encoders to use pseudo-relative mode
    def _on_parameter_encoder_value(self, value, encoder):
        self._notify_modification()
    ...
    def _show_settings(self):
        if self.selected_mode != 'enabled':
            ...
            self._notify_modification()
            self._update_selected_setting(self._mode_selector.selected_mode)

    def _update_selected_setting(self, mode):
        if self.selected_mode == 'enabled':
            if mode == 'note_settings':
                self.selected_setting = 'note_settings'
            elif mode == 'automation':
                self.selected_setting = 'automation'

    def _try_hide_settings(self):
        if self._request_hide and not any(imap(lambda e: e and e.is_pressed(), self._notes_encoders or self._parameter_encoders or [])):
            self.selected_mode = 'disabled'
            self._request_hide = False
